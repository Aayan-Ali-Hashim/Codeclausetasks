...
def send_email(to_entry, subject_entry, message_text):
    ...
    server.starttls()
    server.login('aayanali507@gmail.com', 'vlkqbylzhmphegik')
...
def main():
    ...
    to_entry = ct.CTkEntry(top_frame,height=50,width=400,font=("bold",20))
    ...
    subject_entry = ct.CTkEntry(top_frame,height=50,width=400,font=("bold",20))
    subject_entry.pack(side='left')
    message_label = ct.CTkLabel(bottom_frame, text='Message:', bg_color='green')
    message_label.pack(side='left')
    message_text = ct.CTkTextbox(bottom_frame,height=100,width=500,font=("bold",20))
